...
class cgcnn(base_model):
    ...
    def poly(self, x, L, Fout, K):
        ...
        #L = graph.rescale_L(L, lmax=2) # rescale!!
        L = graph.jacobi_L(L, lmax=2)
        # Transform to Chebyshev basis
        x = tf.transpose(x, perm=[1, 2, 0])  # M x Fin x N
        x = tf.reshape(x, [M, Fin*N])  # M x Fin*N
        def poly(x):
          return graph.poly(L, x, K)
        x = tf.py_func(poly, [x], [tf.float32])[0]  # K x M x Fin*N
